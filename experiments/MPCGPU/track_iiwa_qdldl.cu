...
int main(){
    ...
    for(uint32_t ind = 0; ind < start_goal_combinations; ind++){
		...
		std::string test_output_prefix = output_directory_path  + std::to_string(gato::KNOT_POINTS) + "_" + ( (LINSYS_SOLVE == 1) ? "PCG" : "QDLDL");
		...
		for (uint32_t single_traj_test_iter = 0; single_traj_test_iter < traj_test_iters; single_traj_test_iter++){
			...
			std::tuple<std::vector<toplevel_return_type>, std::vector<linsys_t>, linsys_t> trackingstats = simulateMPC<linsys_t, toplevel_return_type>(static_cast<uint32_t>(eePos_traj2d.size()), 
				d_eePos_traj, d_xu_traj, d_xs, linsys_exit_tol, mpc_log_params);
