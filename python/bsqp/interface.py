...
class BSQP:
    def __init__(self, model_path,
                 batch_size, N, dt, 
                 max_sqp_iters=10, kkt_tol=1e-4, 
                 max_pcg_iters=100, pcg_tol=1e-4,  
                 solve_ratio=0.875, mu=1.0, 
                 q_cost=2.0, qd_cost=1e-4, u_cost=1e-6, N_cost=50.0, q_lim_cost=1e-3,
                 f_ext_B_std=None, f_ext_resample_std=None):
        ...
        try:
            ...
            raise ValueError(f"Number of knots {N} not supported (could not import {module_name}): {e}")

        # Build the class name for the given batch_size
        class_name = f"BSQP_{batch_size}"
        if not hasattr(base, class_name):
            raise ValueError(f"Batch size {batch_size} not supported in module {module_name}")
        self.lib = base
        self.solver_class = getattr(base, class_name)
        
        self.solver = self.solver_class(dt, max_sqp_iters, kkt_tol, 
                                                    max_pcg_iters, pcg_tol, 
                                                    solve_ratio, mu, 
                                                    q_cost, qd_cost, u_cost, N_cost, q_lim_cost)
        ...
        if f_ext_B_std is not None:
            self.f_ext_B[1:, :3] = np.random.normal(0, f_ext_B_std, (self.batch_size-1, 3))
    ...
    def get_best_idx(self, x_last, u_last, x_curr, dt):
        ...
        if self.f_ext_resample_std is not None:
            ...
            self.f_ext_B[2:, :] = np.random.normal(self.f_ext_B[best_id, :], self.f_ext_resample_std, (self.batch_size-2, 6))
            self.f_ext_B[:, 3:] = 0.0
            self.set_f_ext_B(self.f_ext_B)
