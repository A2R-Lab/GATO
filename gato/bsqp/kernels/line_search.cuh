...
__global__ void lineSearchAndUpdateBatchedKernel(T* d_xu_traj_batch, T* d_dz_batch, T* d_merit_batch, T* d_merit_initial_batch, T* d_step_size_batch)
{
        ...
        if (s_merit[0] >= d_merit_initial_batch[solve_idx]) {
                if (tid == 0) { d_step_size_batch[solve_idx] = -1; }
                return;
        }
...
template<typename T, uint32_t BatchSize, uint32_t NumAlphas>
__host__ void lineSearchAndUpdateBatched(T* d_xu_traj_batch, T* d_dz_batch, T* d_merit_batch, T* d_merit_initial_batch, T* d_step_size_batch)
{
        dim3   grid(BatchSize);
        dim3   thread_block(LINE_SEARCH_THREADS);
        size_t s_mem_size = sizeof(T) * NumAlphas + sizeof(uint32_t) * NumAlphas;

        lineSearchAndUpdateBatchedKernel<T, BatchSize, NumAlphas><<<grid, thread_block, s_mem_size>>>(d_xu_traj_batch, d_dz_batch, d_merit_batch, d_merit_initial_batch, d_step_size_batch);
}
