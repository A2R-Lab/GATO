...
// ---- shared tails (monolithic solve + factor/solve split stay byte-for-byte) ----
// Factor the strips in place; on a non-PD pivot scrub the factor's NaN smem
// staging (persists on the SM) and report failure. All threads; ends synced.
template<typename T>
__device__ __forceinline__ bool bdsv_factor_or_scrub(T* d_S_matrix, T* s_mem, int* s_fail)
{
...
// x <- -rhs (pads stay 0), then the factored solve in place. Ends synced.
template<typename T>
__device__ __forceinline__ void bdsv_seed_and_solve(const T* d_S_matrix, const T* d_rhs_vector, T* d_x_vector, T* s_mem)
{
        const uint32_t rank = threadIdx.x, size = blockDim.x;
        for (uint32_t i = rank; i < VEC_SIZE_PADDED; i += size) { d_x_vector[i] = -d_rhs_vector[i]; }
        __syncthreads();
                                                                       ...
                                                                       const int32_t* __restrict__ d_kkt_converged_batch)
{
        ...
        if (!bdsv_factor_or_scrub<T>(d_S_matrix, s_mem, &s_fail)) {  // skip update: λ untouched; strips are rewritten next iteration
                if (rank == 0) { d_iterations[solve_idx] = 2; }
                return;
        }
        bdsv_seed_and_solve<T>(d_S_matrix, d_b_vector, d_x_vector, s_mem);   // λ ← −γ, then solve
...
constexpr int32_t SKIPPED = 2;  // solve was converged: strips still hold UN-factored S
}  // namespace bdsv_status
                                                                        ...
                                                                        const int32_t* __restrict__ d_kkt_converged_batch)
{
        ...
        __shared__ int s_fail;
        if (!bdsv_factor_or_scrub<T>(d_S_matrix, s_mem, &s_fail)) {
