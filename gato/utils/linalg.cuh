...
template <typename T, uint32_t dim>
__device__ __forceinline__
void addScaledIdentity(T *A, T alpha) {
    #pragma unroll
    for (uint32_t i = threadIdx.x; i < dim * dim; i += blockDim.x) {
        uint32_t x = i / dim;
        uint32_t y = i % dim;
...
__device__ __forceinline__
void matMul(T *C, T *A, T *B, bool negate = false) {
...
template <typename T, uint32_t m, uint32_t n, uint32_t k>
__device__ __forceinline__
void matMulSum(T *C, T *A, T *B, bool negate = false) {
    // threads each compute an element of C
    uint32_t x, y;
    T sum;
    #pragma unroll
...
template <typename T, uint32_t m, uint32_t n, uint32_t k>
__device__ __forceinline__
void matMulTranspose(T *C, T *A, T *B, bool negate = false) {
    // threads each compute an element of C
    uint32_t x, y;
    T sum;
    #pragma unroll
...
template <typename T, uint32_t m, uint32_t n, uint32_t k>
__device__ __forceinline__
void matMulTransposeSum(T *C, T *A, T *B, bool negate = false) {
    // threads each compute an element of C
    uint32_t x, y;
    T sum;
    #pragma unroll
...
template <typename T>
__device__  __forceinline__
void reduce(const uint32_t n, T *x)
{
    ...
    while (size_left > 3){
        ...
        // reduce in half
        for (uint32_t i = idx; i < size_left; i += stride){
            x[i] += x[i + size_left];
        }	
...
template <typename T, uint32_t BatchSize>
__device__ __forceinline__
T *getOffsetState(T *batch, uint32_t solve_idx, uint32_t knot_idx) {
...
template <typename T, uint32_t BatchSize>
__device__ __forceinline__
T *getOffsetControl(T *batch, uint32_t solve_idx, uint32_t knot_idx) {
...
template <typename T, uint32_t BatchSize>
__device__ __forceinline__
T *getOffsetStateSq(T *batch, uint32_t solve_idx, uint32_t knot_idx) {
...
template <typename T, uint32_t BatchSize>
__device__ __forceinline__
T *getOffsetControlSq(T *batch, uint32_t solve_idx, uint32_t knot_idx) {
    return batch + solve_idx * CONTROL_SQ_P_KNOTS + knot_idx * CONTROL_SIZE_SQ;
}

template <typename T, uint32_t BatchSize>
__device__ __forceinline__
T *getOffsetStatePControl(T *batch, uint32_t solve_idx, uint32_t knot_idx) {
...
template <typename T, uint32_t BatchSize>
__device__ __forceinline__
T *getOffsetTraj(T *batch, uint32_t solve_idx, uint32_t knot_idx) {
    return batch + solve_idx * TRAJ_SIZE + knot_idx * STATE_S_CONTROL;
}

template <typename T, uint32_t BatchSize>
__device__ __forceinline__
T *getOffsetReferenceTraj(T *batch, uint32_t solve_idx, uint32_t knot_idx) {
...
template <typename T, uint32_t BatchSize>
__device__ __forceinline__
T *getOffsetStatePadded(T *batch, uint32_t solve_idx, uint32_t knot_idx) {
    return batch + solve_idx * VEC_SIZE_PADDED + (knot_idx + 1) * STATE_SIZE;
}

template <typename T, uint32_t BatchSize>
__device__ __forceinline__
T *getOffsetBlockRowPadded(T *batch, uint32_t solve_idx, uint32_t knot_idx) {
