...
#include "kernels/setup_schur_qdldl.cuh"
#include "qdldl.h"
...
template <typename T>
auto sqpSolveQdldl(T *d_eePos_traj, T *d_xu, T *d_lambda, void *d_dynMem_const, T &rho, T rho_reset){
    ...
    void *ls_merit_kernel = (void *) ls_compute_merit_kernel<T>;
    ...
    for(uint32_t sqpiter = 0; sqpiter < SQP_MAX_ITER; sqpiter++){
        ...
        setup_kkt_kernel<T><<<knot_points, KKT_THREADS, 2 * get_kkt_kernel_smem_size<T>()>>>(
            d_G_dense, 
            d_C_dense, 
            d_g, 
            d_c,
        ...
        form_schur_system_qdldl<T>(d_G_dense, d_C_dense, d_g, d_c, d_val, d_gamma, rho);
        gpuErrchk(cudaPeekAtLastError());
        if (sqpTimecheck()){ break; }
        ...
        for(uint32_t p = 0; p < num_alphas; p++){
            ...
            gpuErrchk(cudaLaunchCooperativeKernel(ls_merit_kernel, knot_points, MERIT_THREADS, kernelArgs, merit_smem_size, streams[p]));
        }
        if (sqpTimecheck()){ break; }
        gpuErrchk(cudaPeekAtLastError());
        gpuErrchk(cudaDeviceSynchronize());
