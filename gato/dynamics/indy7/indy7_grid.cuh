 ...
 *       __host__   inverse_dynamics<T,USE_QDD_FLAG=false,USE_COMPRESSED_MEM=false>(gridData<T> *hd_data, const robotModel<T> *d_robotModel, const T gravity, const int num_timesteps, const dim3
 * block_dimms, const dim3 thread_dimms, cudaStream_t *streams)
 *
 ...
 *       __host__   direct_minv<T,USE_COMPRESSED_MEM=false>(gridData<T> *hd_data, const robotModel<T> *d_robotModel, const int num_timesteps, const dim3 block_dimms, const dim3 thread_dimms,
 ...
 *       __host__   forward_dynamics<T>(gridData<T> *hd_data, const robotModel<T> *d_robotModel, const T gravity, const int num_timesteps, const dim3 block_dimms, const dim3 thread_dimms, cudaStream_t
 * *streams)
 *
 ...
 *       __host__   inverse_dynamics_gradient<T,USE_QDD_FLAG=false,USE_COMPRESSED_MEM=false>(gridData<T> *hd_data, const robotModel<T> *d_robotModel, const T gravity, const int num_timesteps, const
 * dim3 block_dimms, const dim3 thread_dimms, cudaStream_t *streams)
 *
 ...
 *       __host__   forward_dynamics_gradient<T,USE_QDD_MINV_FLAG=false>(gridData<T> *hd_data, const robotModel<T> *d_robotModel, const T gravity, const int num_timesteps, const dim3 block_dimms,
 * const dim3 thread_dimms, cudaStream_t *streams)
 *
 ...
 *       __host__   end_effector_positions<T,USE_COMPRESSED_MEM=false>(gridData<T> *hd_data, const robotModel<T> *d_robotModel, const int num_timesteps, const dim3 block_dimms, const dim3
 * thread_dimms, cudaStream_t *streams)
 *
 ...
 *       __host__   end_effector_positions_gradient<T,USE_COMPRESSED_MEM=false>(gridData<T> *hd_data, const robotModel<T> *d_robotModel, const int num_timesteps, const dim3 block_dimms, const dim3
 * thread_dimms, cudaStream_t *streams)
 *
...
// __host__ __device__ void printMat(T* A, int lda)
// {
//         for (int i = 0; i < M; i++) {
...
// template<typename T, int M, int N>
// __host__ __device__ void printMat(const T* A, int lda)
// {
//         for (int i = 0; i < M; i++) {
//                 for (int j = 0; j < N; j++) { printf("%.4f ", A[i + lda * j]); }
//                 printf("\n");
...
constexpr int ID_DYNAMIC_SHARED_MEM_COUNT = 660;
constexpr int MINV_DYNAMIC_SHARED_MEM_COUNT = 1170;
constexpr int FD_DYNAMIC_SHARED_MEM_COUNT = 1356;
constexpr int ID_DU_DYNAMIC_SHARED_MEM_COUNT = 1956;
constexpr int FD_DU_DYNAMIC_SHARED_MEM_COUNT = 1956;
constexpr int ID_DU_MAX_SHARED_MEM_COUNT = 2154;
constexpr int FD_DU_MAX_SHARED_MEM_COUNT = 2268;
constexpr int EE_POS_DYNAMIC_SHARED_MEM_COUNT = 128;
constexpr int DEE_POS_DYNAMIC_SHARED_MEM_COUNT = 384;
constexpr int SUGGESTED_THREADS = 256;
...
__device__ T dot_prod(const T* vec1, const T* vec2)
{
        T result = 0;
 ...
 */
template<typename T, int N, int S1, int S2>
__device__ T dot_prod(T* vec1, const T* vec2)
{
        T result = 0;
        for (int i = 0; i < N; i++) { result += vec1[i * S1] * vec2[i * S2]; }
        return result;
 ...
 */
template<typename T, int N, int S1, int S2>
__device__ T dot_prod(const T* vec1, T* vec2)
{
        T result = 0;
        for (int i = 0; i < N; i++) { result += vec1[i * S1] * vec2[i * S2]; }
        return result;
 ...
 */
template<typename T, int N, int S1, int S2>
__device__ T dot_prod(T* vec1, T* vec2)
{
        T result = 0;
        for (int i = 0; i < N; i++) { result += vec1[i * S1] * vec2[i * S2]; }
        return result;
 ...
 */
template<typename T>
__device__ void mx0(T* s_vecX, const T* s_vec)
{
 ...
 */
template<typename T>
__device__ void mx0_peq(T* s_vecX, const T* s_vec)
{
        s_vecX[1] += s_vec[2];
        s_vecX[2] += -s_vec[1];
        s_vecX[4] += s_vec[5];
 ...
 */
template<typename T>
__device__ void mx0_scaled(T* s_vecX, const T* s_vec, const T alpha)
{
        s_vecX[0] = static_cast<T>(0);
 ...
 */
template<typename T>
__device__ void mx0_peq_scaled(T* s_vecX, const T* s_vec, const T alpha)
{
        s_vecX[1] += s_vec[2] * alpha;
        s_vecX[2] += -s_vec[1] * alpha;
        s_vecX[4] += s_vec[5] * alpha;
 ...
 */
template<typename T>
__device__ void mx1(T* s_vecX, const T* s_vec)
{
        s_vecX[0] = -s_vec[2];
 ...
 */
template<typename T>
__device__ void mx1_peq(T* s_vecX, const T* s_vec)
{
        s_vecX[0] += -s_vec[2];
        s_vecX[2] += s_vec[0];
        s_vecX[3] += -s_vec[5];
 ...
 */
template<typename T>
__device__ void mx1_scaled(T* s_vecX, const T* s_vec, const T alpha)
{
 ...
 */
template<typename T>
__device__ void mx1_peq_scaled(T* s_vecX, const T* s_vec, const T alpha)
{
        s_vecX[0] += -s_vec[2] * alpha;
        s_vecX[2] += s_vec[0] * alpha;
        s_vecX[3] += -s_vec[5] * alpha;
 ...
 */
template<typename T>
__device__ void mx2(T* s_vecX, const T* s_vec)
{
        s_vecX[0] = s_vec[1];
        s_vecX[1] = -s_vec[0];
 ...
 */
template<typename T>
__device__ void mx2_peq(T* s_vecX, const T* s_vec)
{
        s_vecX[0] += s_vec[1];
        s_vecX[1] += -s_vec[0];
        s_vecX[3] += s_vec[4];
 ...
 */
template<typename T>
__device__ void mx2_scaled(T* s_vecX, const T* s_vec, const T alpha)
{
 ...
 */
template<typename T>
__device__ void mx2_peq_scaled(T* s_vecX, const T* s_vec, const T alpha)
{
        s_vecX[0] += s_vec[1] * alpha;
        s_vecX[1] += -s_vec[0] * alpha;
        s_vecX[3] += s_vec[4] * alpha;
 ...
 */
template<typename T>
__device__ void mx3(T* s_vecX, const T* s_vec)
{
        s_vecX[0] = static_cast<T>(0);
        s_vecX[1] = static_cast<T>(0);
        s_vecX[2] = static_cast<T>(0);
 ...
 */
template<typename T>
__device__ void mx3_peq(T* s_vecX, const T* s_vec)
{
        s_vecX[4] += s_vec[2];
        s_vecX[5] += -s_vec[1];
}
 ...
 */
template<typename T>
__device__ void mx3_scaled(T* s_vecX, const T* s_vec, const T alpha)
{
        s_vecX[0] = static_cast<T>(0);
        s_vecX[1] = static_cast<T>(0);
        s_vecX[2] = static_cast<T>(0);
 ...
 */
template<typename T>
__device__ void mx3_peq_scaled(T* s_vecX, const T* s_vec, const T alpha)
{
        s_vecX[4] += s_vec[2] * alpha;
        s_vecX[5] += -s_vec[1] * alpha;
}
 ...
 */
template<typename T>
__device__ void mx4(T* s_vecX, const T* s_vec)
{
        s_vecX[0] = static_cast<T>(0);
        s_vecX[1] = static_cast<T>(0);
        s_vecX[2] = static_cast<T>(0);
 ...
 */
template<typename T>
__device__ void mx4_peq(T* s_vecX, const T* s_vec)
{
        s_vecX[3] += -s_vec[2];
        s_vecX[5] += s_vec[0];
}
 ...
 */
template<typename T>
__device__ void mx4_scaled(T* s_vecX, const T* s_vec, const T alpha)
{
        s_vecX[0] = static_cast<T>(0);
        s_vecX[1] = static_cast<T>(0);
        s_vecX[2] = static_cast<T>(0);
 ...
 */
template<typename T>
__device__ void mx4_peq_scaled(T* s_vecX, const T* s_vec, const T alpha)
{
        s_vecX[3] += -s_vec[2] * alpha;
        s_vecX[5] += s_vec[0] * alpha;
}
 ...
 */
template<typename T>
__device__ void mx5(T* s_vecX, const T* s_vec)
{
        s_vecX[0] = static_cast<T>(0);
        s_vecX[1] = static_cast<T>(0);
        s_vecX[2] = static_cast<T>(0);
 ...
 */
template<typename T>
__device__ void mx5_peq(T* s_vecX, const T* s_vec)
{
        s_vecX[3] += s_vec[1];
        s_vecX[4] += -s_vec[0];
}
 ...
 */
template<typename T>
__device__ void mx5_scaled(T* s_vecX, const T* s_vec, const T alpha)
{
        s_vecX[0] = static_cast<T>(0);
        s_vecX[1] = static_cast<T>(0);
        s_vecX[2] = static_cast<T>(0);
 ...
 */
template<typename T>
__device__ void mx5_peq_scaled(T* s_vecX, const T* s_vec, const T alpha)
{
        s_vecX[3] += s_vec[1] * alpha;
        s_vecX[4] += -s_vec[0] * alpha;
}
 ...
 */
template<typename T>
__device__ void mxX(T* s_vecX, const T* s_vec, const int S_ind)
{
        switch (S_ind) {
        case 0:
                mx0<T>(s_vecX, s_vec);
 ...
 */
template<typename T>
__device__ void mxX_peq(T* s_vecX, const T* s_vec, const int S_ind)
{
        switch (S_ind) {
        case 0:
                mx0_peq<T>(s_vecX, s_vec);
                break;
        case 1:
                mx1_peq<T>(s_vecX, s_vec);
                break;
        case 2:
                mx2_peq<T>(s_vecX, s_vec);
                break;
        case 3:
                mx3_peq<T>(s_vecX, s_vec);
                break;
        case 4:
                mx4_peq<T>(s_vecX, s_vec);
                break;
        case 5:
                mx5_peq<T>(s_vecX, s_vec);
                break;
        }
}
 ...
 */
template<typename T>
__device__ void mxX_scaled(T* s_vecX, const T* s_vec, const T alpha, const int S_ind)
{
        switch (S_ind) {
        case 0:
                mx0_scaled<T>(s_vecX, s_vec, alpha);
                break;
        case 1:
                mx1_scaled<T>(s_vecX, s_vec, alpha);
                break;
        case 2:
                mx2_scaled<T>(s_vecX, s_vec, alpha);
                break;
        case 3:
                mx3_scaled<T>(s_vecX, s_vec, alpha);
                break;
        case 4:
                mx4_scaled<T>(s_vecX, s_vec, alpha);
                break;
        case 5:
                mx5_scaled<T>(s_vecX, s_vec, alpha);
                break;
        }
}
 ...
 */
template<typename T>
__device__ void mxX_peq_scaled(T* s_vecX, const T* s_vec, const T alpha, const int S_ind)
{
        switch (S_ind) {
        case 0:
                mx0_peq_scaled<T>(s_vecX, s_vec, alpha);
                break;
        case 1:
                mx1_peq_scaled<T>(s_vecX, s_vec, alpha);
                break;
        case 2:
                mx2_peq_scaled<T>(s_vecX, s_vec, alpha);
                break;
        case 3:
                mx3_peq_scaled<T>(s_vecX, s_vec, alpha);
                break;
        case 4:
                mx4_peq_scaled<T>(s_vecX, s_vec, alpha);
                break;
        case 5:
                mx5_peq_scaled<T>(s_vecX, s_vec, alpha);
                break;
        }
}
 ...
 */
template<typename T>
__device__ void fx(T* s_matX, const T* s_vecX)
{
 ...
 */
template<typename T>
__device__ void fx_zeroed(T* s_matX, const T* s_vecX)
{
        s_matX[6 * 0 + 1] = s_vecX[2];
        s_matX[6 * 0 + 2] = -s_vecX[1];
        s_matX[6 * 1 + 0] = -s_vecX[2];
 ...
 */
template<typename T>
__device__ void fx_times_v(T* s_result, const T* s_fxVec, const T* s_timesVec)
{
 ...
 */
template<typename T>
__device__ void fx_times_v_peq(T* s_result, const T* s_fxVec, const T* s_timesVec)
{
        s_result[0] += -s_fxVec[2] * s_timesVec[1] + s_fxVec[1] * s_timesVec[2] - s_fxVec[5] * s_timesVec[4] + s_fxVec[4] * s_timesVec[5];
        s_result[1] += s_fxVec[2] * s_timesVec[0] - s_fxVec[0] * s_timesVec[2] + s_fxVec[5] * s_timesVec[3] - s_fxVec[3] * s_timesVec[5];
        s_result[2] += -s_fxVec[1] * s_timesVec[0] + s_fxVec[0] * s_timesVec[1] - s_fxVec[4] * s_timesVec[3] + s_fxVec[3] * s_timesVec[4];
...
//
template<typename T>
__host__ int* init_topology_helpers()
{
        return nullptr;
}
/**
...
__host__ T* init_XImats()
{
        ...
        T* d_XImats;
        gpuErrchk(cudaMalloc((void**)&d_XImats, 624 * sizeof(T)));
 ...
 */
template<typename T>
__host__ robotModel<T>* init_robotModel()
{
        ...
        gpuErrchk(cudaMalloc((void**)&d_robotModel, sizeof(robotModel<T>)));
 ...
 */
template<typename T>
__device__ void load_update_XImats_helpers(T* s_XImats, const T* s_q, const robotModel<T>* d_robotModel, T* s_temp)
{
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 432; ind += blockDim.x * blockDim.y) { s_XImats[ind] = d_robotModel->d_XImats[ind]; }
        ...
        for (int kcr = threadIdx.x + threadIdx.y * blockDim.x; kcr < 54; kcr += blockDim.x * blockDim.y) {
                ...
                int c = cr / 3;
                int r = cr % 3;
                int srcInd = k * 36 + c * 6 + r;
                int dstInd = srcInd + 21;  // 3 more rows and cols
 ...
 */
template<typename T>
__device__ void load_update_XmatsHom_helpers(T* s_XmatsHom, const T* s_q, const robotModel<T>* d_robotModel, T* s_temp)
{
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 96; ind += blockDim.x * blockDim.y) { s_XmatsHom[ind] = d_robotModel->d_XImats[ind + 432]; }
        for (int k = threadIdx.x + threadIdx.y * blockDim.x; k < 6; k += blockDim.x * blockDim.y) {
                s_temp[k] = static_cast<T>(sin(s_q[k]));
                s_temp[k + 6] = static_cast<T>(cos(s_q[k]));
        }
 ...
 */
template<typename T>
__device__ void load_update_XmatsHom_helpers(T* s_XmatsHom, T* s_dXmatsHom, const T* s_q, const robotModel<T>* d_robotModel, T* s_temp)
{
 ...
 */
template<typename T>
__device__ void end_effector_positions_inner(T* s_eePos, const T* s_q, const T* s_Xhom, T* s_temp)
{
        //
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 16; ind += blockDim.x * blockDim.y) { s_temp[ind] = s_Xhom[16 * 5 + ind]; }
        __syncthreads();
        // Serial chain manipulator so optimize as parent is jid-1
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 3; ind += blockDim.x * blockDim.y) {
                ...
                int xyzInd = ind % 3;
                int eeInd = ind / 3;
                T*  s_Xmat_hom = &s_temp[16 + 16 * eeInd];
 ...
 */
template<typename T>
__device__ void end_effector_positions_device(T* s_eePos, const T* s_q, T* s_temp_in, const robotModel<T>* d_robotModel)
{
        T* s_XHomTemp = s_temp_in;
        T* s_XmatsHom = s_XHomTemp;
        T* s_temp = &s_XHomTemp[96];
 ...
 */
template<typename T>
__device__ void end_effector_positions_gradient_inner(T* s_deePos, const T* s_q, const T* s_Xhom, const T* s_dXhom, T* s_temp)
{
        //
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 18; ind += blockDim.x * blockDim.y) {
                ...
                int xyzInd = ind % 3;
                int deeInd = ind / 3;
                T*  s_Xmat_hom = &s_temp[96 + 16 * deeInd];
 ...
 */
template<typename T>
__device__ void end_effector_positions_gradient_device(T* s_deePos, const T* s_q, T* s_temp_in, const robotModel<T>* d_robotModel)
{
        T* s_XHomTemp = s_temp_in;
        T* s_XmatsHom = s_XHomTemp;
        T* s_dXmatsHom = &s_XHomTemp[96];
        T* s_temp = &s_dXmatsHom[96];
 ...
 */
template<typename T>
__device__ void inverse_dynamics_inner(T* s_c, T* s_vaf, const T* s_q, const T* s_qd, const T* s_qdd, T* s_XImats, T* s_temp, const T gravity)
{
        //
        // Forward Pass
        //
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 12; ind += blockDim.x * blockDim.y) {
                ...
                int comp = ind / 6;
                int comp_mod = comp % 1;
                int vFlag = comp == comp_mod;
                int vaOffset = !vFlag * 36;
                int jid6 = 6 * 1;
        ...
        __syncthreads();
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 1; ind += blockDim.x * blockDim.y) { mx2_peq_scaled<T>(&s_vaf[42], &s_vaf[6], s_qd[1]); }
        __syncthreads();
        ...
        // sync before a += MxS(v)*qd[S]
        __syncthreads();
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 1; ind += blockDim.x * blockDim.y) { mx2_peq_scaled<T>(&s_vaf[48], &s_vaf[12], s_qd[2]); }
        __syncthreads();
        ...
        // sync before a += MxS(v)*qd[S]
        __syncthreads();
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 1; ind += blockDim.x * blockDim.y) { mx2_peq_scaled<T>(&s_vaf[54], &s_vaf[18], s_qd[3]); }
        __syncthreads();
        ...
        // sync before a += MxS(v)*qd[S]
        __syncthreads();
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 1; ind += blockDim.x * blockDim.y) { mx2_peq_scaled<T>(&s_vaf[60], &s_vaf[24], s_qd[4]); }
        __syncthreads();
        ...
        // sync before a += MxS(v)*qd[S]
        __syncthreads();
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 1; ind += blockDim.x * blockDim.y) { mx2_peq_scaled<T>(&s_vaf[66], &s_vaf[30], s_qd[5]); }
        __syncthreads();
        //
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 72; ind += blockDim.x * blockDim.y) {
                ...
                int  comp = ind / 6;
                int  jid = comp % 6;
                bool IaFlag = comp == jid;
                int  jid6 = 6 * jid;
                int  vaOffset = IaFlag * 36 + jid6;
        ...
        //
        for (int jid = threadIdx.x + threadIdx.y * blockDim.x; jid < 6; jid += blockDim.x * blockDim.y) { s_c[jid] = s_vaf[72 + 6 * jid + 2]; }
        __syncthreads();
}
 ...
 */
template<typename T>
__device__ void inverse_dynamics_inner(T* s_c, T* s_vaf, const T* s_q, const T* s_qd, T* s_XImats, T* s_temp, const T gravity)
{
        //
        // Forward Pass
        //
 ...
 */
template<typename T>
__device__ void inverse_dynamics_inner_vaf(T* s_vaf, const T* s_q, const T* s_qd, const T* s_qdd, T* s_XImats, T* s_temp, const T gravity)
{
        //
        // Forward Pass
        //
 ...
 */
template<typename T>
__device__ void inverse_dynamics_inner_vaf(T* s_vaf, const T* s_q, const T* s_qd, T* s_XImats, T* s_temp, const T gravity)
{
        //
        // Forward Pass
        //
 ...
 */
template<typename T>
__device__ void direct_minv_inner(T* s_Minv, const T* s_q, T* s_XImats, T* s_temp)
{
        // T *s_F = &s_temp[0]; T *s_IA = &s_temp[216]; T *s_U = &s_temp[432]; T *s_Dinv = &s_temp[468]; T *s_Ia = &s_temp[474]; T *s_IaTemp = &s_temp[510];
        // Initialize IA = I
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 216; ind += blockDim.x * blockDim.y) { s_temp[216 + ind] = s_XImats[216 + ind]; }
        // Zero Minv and F
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 252; ind += blockDim.x * blockDim.y) {
                if (ind < 216) {
                        s_temp[0 + ind] = static_cast<T>(0);
                } else {
                        s_Minv[ind - 216] = static_cast<T>(0);
                }
        }
        __syncthreads();
        //
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 1; ind += blockDim.x * blockDim.y) {
                ...
                for (int row = 0; row < 6; row++) { s_temp[0 + 36 * 5 + 30 + row] += s_temp[432 + 6 * 5 + row] * s_Minv[30 + 5]; }
        }
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 42; ind += blockDim.x * blockDim.y) {
                int row = ind % 6;
                int col = ind / 6;
                T*  src = &s_temp[0 + 36 * 5 + 6 * 5];
                T*  dst = &s_temp[0 + 36 * 4 + 6 * 5];
                // adjust for temp comps
                if (col >= 1) {
                        col -= 1;
                        src = &s_temp[474 + 6 * col];
                        dst = &s_temp[510 + 6 * col];
                }
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 2; ind += blockDim.x * blockDim.y) {
                int jid_subtree6 = 6 * (4 + ind);
                int jid_subtreeN = 6 * (4 + ind);
                s_Minv[jid_subtreeN + 4] -= s_temp[468 + 4] * s_temp[0 + 36 * 4 + jid_subtree6 + 2];
                for (int row = 0; row < 6; row++) { s_temp[0 + 36 * 4 + jid_subtree6 + row] += s_temp[432 + 6 * 4 + row] * s_Minv[jid_subtreeN + 4]; }
        }
        // Ia = IA - U^T Dinv U | to start IAparent Update
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 36; ind += blockDim.x * blockDim.y) {
                int row = ind % 6;
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 48; ind += blockDim.x * blockDim.y) {
                int row = ind % 6;
                int col = ind / 6;
                T*  src = &s_temp[0 + 36 * 4 + 6 * (4 + col)];
                T*  dst = &s_temp[0 + 36 * 3 + 6 * (4 + col)];
                // adjust for temp comps
                if (col >= 2) {
                        col -= 2;
                        src = &s_temp[474 + 6 * col];
        ...
        // Temp Comp: F[i,:,subTreeInds] += U*Minv[i,subTreeInds] - to start Fparent Update
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 3; ind += blockDim.x * blockDim.y) {
                int jid_subtree6 = 6 * (3 + ind);
                int jid_subtreeN = 6 * (3 + ind);
                s_Minv[jid_subtreeN + 3] -= s_temp[468 + 3] * s_temp[0 + 36 * 3 + jid_subtree6 + 2];
                for (int row = 0; row < 6; row++) { s_temp[0 + 36 * 3 + jid_subtree6 + row] += s_temp[432 + 6 * 3 + row] * s_Minv[jid_subtreeN + 3]; }
        }
        // Ia = IA - U^T Dinv U | to start IAparent Update
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 36; ind += blockDim.x * blockDim.y) {
                int row = ind % 6;
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 54; ind += blockDim.x * blockDim.y) {
                int row = ind % 6;
                int col = ind / 6;
                T*  src = &s_temp[0 + 36 * 3 + 6 * (3 + col)];
                T*  dst = &s_temp[0 + 36 * 2 + 6 * (3 + col)];
                // adjust for temp comps
                if (col >= 3) {
                        col -= 3;
                        src = &s_temp[474 + 6 * col];
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 4; ind += blockDim.x * blockDim.y) {
                int jid_subtree6 = 6 * (2 + ind);
                int jid_subtreeN = 6 * (2 + ind);
                s_Minv[jid_subtreeN + 2] -= s_temp[468 + 2] * s_temp[0 + 36 * 2 + jid_subtree6 + 2];
                for (int row = 0; row < 6; row++) { s_temp[0 + 36 * 2 + jid_subtree6 + row] += s_temp[432 + 6 * 2 + row] * s_Minv[jid_subtreeN + 2]; }
        }
        // Ia = IA - U^T Dinv U | to start IAparent Update
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 36; ind += blockDim.x * blockDim.y) {
                int row = ind % 6;
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 60; ind += blockDim.x * blockDim.y) {
                int row = ind % 6;
                int col = ind / 6;
                T*  src = &s_temp[0 + 36 * 2 + 6 * (2 + col)];
                T*  dst = &s_temp[0 + 36 * 1 + 6 * (2 + col)];
                // adjust for temp comps
                if (col >= 4) {
                        col -= 4;
                        src = &s_temp[474 + 6 * col];
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 5; ind += blockDim.x * blockDim.y) {
                int jid_subtree6 = 6 * (1 + ind);
                int jid_subtreeN = 6 * (1 + ind);
                s_Minv[jid_subtreeN + 1] -= s_temp[468 + 1] * s_temp[0 + 36 * 1 + jid_subtree6 + 2];
                for (int row = 0; row < 6; row++) { s_temp[0 + 36 * 1 + jid_subtree6 + row] += s_temp[432 + 6 * 1 + row] * s_Minv[jid_subtreeN + 1]; }
        }
        // Ia = IA - U^T Dinv U | to start IAparent Update
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 36; ind += blockDim.x * blockDim.y) {
                int row = ind % 6;
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 66; ind += blockDim.x * blockDim.y) {
                int row = ind % 6;
                int col = ind / 6;
                T*  src = &s_temp[0 + 36 * 1 + 6 * (1 + col)];
                T*  dst = &s_temp[0 + 36 * 0 + 6 * (1 + col)];
                // adjust for temp comps
                if (col >= 5) {
                        col -= 5;
                        src = &s_temp[474 + 6 * col];
        ...
        // Minv[i,subTreeInds] -= Dinv*F[i,Srow,SubTreeInds]
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 6; ind += blockDim.x * blockDim.y) {
                int jid_subtree6 = 6 * (0 + ind);
                int jid_subtreeN = 6 * (0 + ind);
 ...
 */
template<typename T>
__device__ void forward_dynamics_finish(T* s_qdd, const T* s_u, const T* s_c, const T* s_Minv)
{
        for (int row = threadIdx.x + threadIdx.y * blockDim.x; row < 6; row += blockDim.x * blockDim.y) {
 ...
 */
template<typename T>
__device__ void forward_dynamics_inner(T* s_qdd, const T* s_q, const T* s_qd, const T* s_u, T* s_XImats, T* s_temp, const T gravity)
{
 ...
 */
template<typename T>
__device__ void inverse_dynamics_gradient_inner(T* s_dc_du, const T* s_q, const T* s_qd, const T* s_vaf, T* s_XImats, T* s_temp, const T gravity)
{
        //
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 108; ind += blockDim.x * blockDim.y) {
                ...
                s_temp[dstOffset] = (parentIsBase && !comp1) ? comp3 * s_XImats[XIOffset + 30] * gravity : dot_prod<T, 6, 6, 1>(&s_XImats[XIOffset], &s_vaf[vaOffset]);
        }
        __syncthreads();
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 12; ind += blockDim.x * blockDim.y) {
                ...
                int col = ind / 6;
                int col_du = col % 1;
                int col_jid = col_du % 1;
                int dq_flag = col < 1;
                int du_col_offset = dq_flag * 0 + !dq_flag * 126 + 6 * col_jid;
                s_temp[du_col_offset + 6 * 1 + row] = dot_prod<T, 6, 6, 1>(&s_XImats[36 * 1 + row], &s_temp[du_col_offset + 6 * 0]);
                // then add {Mx(Xv) or S for col ind}
                s_temp[du_col_offset + 6 * 1 + 6 + row] = dq_flag * s_temp[1152 + 6 * 1 + row] + (!dq_flag && row == 2) * static_cast<T>(1);
        }
        __syncthreads();
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 24; ind += blockDim.x * blockDim.y) {
                ...
                int col = ind / 6;
                int col_du = col % 2;
                int col_jid = col_du % 2;
                int dq_flag = col == col_du;
                int du_col_offset = dq_flag * 0 + !dq_flag * 126 + 6 * col_jid;
                s_temp[du_col_offset + 6 * 3 + row] = dot_prod<T, 6, 6, 1>(&s_XImats[36 * 2 + row], &s_temp[du_col_offset + 6 * 1]);
                // then add {Mx(Xv) or S for col ind}
                if (col_jid == 1) { s_temp[du_col_offset + 6 * 3 + 6 + row] = dq_flag * s_temp[1152 + 6 * 2 + row] + (!dq_flag && row == 2) * static_cast<T>(1); }
        }
        __syncthreads();
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 36; ind += blockDim.x * blockDim.y) {
                ...
                int col = ind / 6;
                int col_du = col % 3;
                int col_jid = col_du % 3;
                int dq_flag = col == col_du;
                int du_col_offset = dq_flag * 0 + !dq_flag * 126 + 6 * col_jid;
                s_temp[du_col_offset + 6 * 6 + row] = dot_prod<T, 6, 6, 1>(&s_XImats[36 * 3 + row], &s_temp[du_col_offset + 6 * 3]);
                // then add {Mx(Xv) or S for col ind}
                if (col_jid == 2) { s_temp[du_col_offset + 6 * 6 + 6 + row] = dq_flag * s_temp[1152 + 6 * 3 + row] + (!dq_flag && row == 2) * static_cast<T>(1); }
        }
        __syncthreads();
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 48; ind += blockDim.x * blockDim.y) {
                ...
                int col = ind / 6;
                int col_du = col % 4;
                int col_jid = col_du % 4;
                int dq_flag = col == col_du;
                int du_col_offset = dq_flag * 0 + !dq_flag * 126 + 6 * col_jid;
                s_temp[du_col_offset + 6 * 10 + row] = dot_prod<T, 6, 6, 1>(&s_XImats[36 * 4 + row], &s_temp[du_col_offset + 6 * 6]);
                // then add {Mx(Xv) or S for col ind}
                if (col_jid == 3) { s_temp[du_col_offset + 6 * 10 + 6 + row] = dq_flag * s_temp[1152 + 6 * 4 + row] + (!dq_flag && row == 2) * static_cast<T>(1); }
        }
        __syncthreads();
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 60; ind += blockDim.x * blockDim.y) {
                ...
                int col = ind / 6;
                int col_du = col % 5;
                int col_jid = col_du % 5;
                int dq_flag = col == col_du;
                int du_col_offset = dq_flag * 0 + !dq_flag * 126 + 6 * col_jid;
                s_temp[du_col_offset + 6 * 15 + row] = dot_prod<T, 6, 6, 1>(&s_XImats[36 * 5 + row], &s_temp[du_col_offset + 6 * 10]);
                // then add {Mx(Xv) or S for col ind}
                if (col_jid == 4) { s_temp[du_col_offset + 6 * 15 + 6 + row] = dq_flag * s_temp[1152 + 6 * 5 + row] + (!dq_flag && row == 2) * static_cast<T>(1); }
        }
        __syncthreads();
        ...
        for (int col = threadIdx.x + threadIdx.y * blockDim.x; col < 42; col += blockDim.x * blockDim.y) {
                ...
                int jid =
                    (col_du < 1) * 0 + (col_du < 3 && col_du >= 1) * 1 + (col_du < 6 && col_du >= 3) * 2 + (col_du < 10 && col_du >= 6) * 3 + (col_du < 15 && col_du >= 10) * 4 + (col_du >= 15) * 5;
                mx2_scaled<T>(&s_temp[252 + 6 * col], &s_temp[0 + 6 * col], s_qd[jid]);
                // then add {MxXa, Mxv} to the appropriate column
                int dq_flag = col == col_du;
                int src_offset = dq_flag * 1188 + !dq_flag * 1224 + 6 * jid;
                if (col_du == ((jid + 1) * (jid + 2) / 2 - 1)) {
                        for (int row = 0; row < 6; row++) { s_temp[252 + 6 * col + row] += s_temp[src_offset + row]; }
                }
        }
        __syncthreads();
        ...
        __syncthreads();
        // Init df/du to 0
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 432; ind += blockDim.x * blockDim.y) { s_temp[504 + ind] = static_cast<T>(0); }
        __syncthreads();
        ...
        for (int col = threadIdx.x + threadIdx.y * blockDim.x; col < 78; col += blockDim.x * blockDim.y) {
                ...
                T*       dst = &s_temp[Offset_col_du_dst];
                const T* fx_src = &s_temp[Offset_col_du_src];
                const T* mult_src = &s_temp[1296 + 6 * jid];
                // Adjust pointers for temp comps (if applicable)
                if (col >= 42) {
                        int comp = col - 42;
                        int comp_col = comp % 6;  // int jid = comp / 6;
                        int jid6 = comp - comp_col;
                        int jid36_col6 = 6 * jid6 + 6 * comp_col;
                        dst = &s_temp[936 + jid36_col6];
                        fx_src = &s_vaf[jid6];
                        mult_src = &s_XImats[216 + jid36_col6];
                }
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 252; ind += blockDim.x * blockDim.y) {
                ...
                const T* dv_col = &s_temp[0 + col6];
                const T* da_col = &s_temp[252 + col6];
                int      jid36 = 36 * jid;
                const T* I_row = &s_XImats[216 + jid36 + row];
                const T* FxvI_row = &s_temp[936 + jid36 + row];
                // Compute the values
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 72; ind += blockDim.x * blockDim.y) {
                ...
                T   update_val = dot_prod<T, 6, 1, 1>(&s_XImats[36 * 5 + 6 * row], &s_temp[du_col_offset + 6 * 30]) + dq_flag * (col_du == 5) * s_temp[1152 + 6 * 5 + row];
                *dst += update_val;
        }
        __syncthreads();
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 72; ind += blockDim.x * blockDim.y) {
                ...
                T   update_val = dot_prod<T, 6, 1, 1>(&s_XImats[36 * 4 + 6 * row], &s_temp[du_col_offset + 6 * 24]) + dq_flag * (col_du == 4) * s_temp[1152 + 6 * 4 + row];
                *dst += update_val;
        }
        __syncthreads();
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 72; ind += blockDim.x * blockDim.y) {
                ...
                T   update_val = dot_prod<T, 6, 1, 1>(&s_XImats[36 * 3 + 6 * row], &s_temp[du_col_offset + 6 * 18]) + dq_flag * (col_du == 3) * s_temp[1152 + 6 * 3 + row];
                *dst += update_val;
        }
        __syncthreads();
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 72; ind += blockDim.x * blockDim.y) {
                ...
                T   update_val = dot_prod<T, 6, 1, 1>(&s_XImats[36 * 2 + 6 * row], &s_temp[du_col_offset + 6 * 12]) + dq_flag * (col_du == 2) * s_temp[1152 + 6 * 2 + row];
                *dst += update_val;
        }
        __syncthreads();
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 72; ind += blockDim.x * blockDim.y) {
                ...
                T   update_val = dot_prod<T, 6, 1, 1>(&s_XImats[36 * 1 + 6 * row], &s_temp[du_col_offset + 6 * 6]) + dq_flag * (col_du == 1) * s_temp[1152 + 6 * 1 + row];
                *dst += update_val;
        }
        __syncthreads();
        ...
        for (int ind = threadIdx.x + threadIdx.y * blockDim.x; ind < 72; ind += blockDim.x * blockDim.y) {
                int jid = ind % 6;
                int jid_dq_qd = ind / 6;
                int jid_du = jid_dq_qd % 6;
                int dq_flag = jid_du == jid_dq_qd;
