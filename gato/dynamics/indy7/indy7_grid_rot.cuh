/**
 * This instance of grid.cuh is optimized for the urdf: indy
 *
 * Notes:
 *   Interface is:
...
namespace grid {
    ...
    const int ABA_DYNAMIC_SHARED_MEM_COUNT = 1464;
    const int CRBA_SHARED_MEM_COUNT = 1464;
    ...
    const int DEE_POS_DYNAMIC_SHARED_MEM_COUNT = 576;
    ...
    void end_effector_positions_inner(T *s_eePos, const T *s_q, const T *s_Xhom, T *s_temp) {
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("q\n"); printMat<T,1,6>(s_q,1);
            for (int i = 0; i < 6; i++){printf("X[%d]\n",i); printMat<T,4,4>(&s_Xhom[16*i],4);}
        }
        __syncthreads();
        //
     ...
     * @param s_temp is a pointer to helper shared memory of size 384
     */
    template <typename T>
    __device__
    void end_effector_positions_gradient_inner(T *s_deePos, const T *s_q, const T *s_Xhom, const T *s_dXhom, T *s_temp) {
        ...
        // Keep chaining until reaching the root (starting from the leaves)
        //
        T *s_eeTemp = &s_temp[0]; T *s_deeTemp = &s_temp[192];
        // Serial chain manipulator so optimize as parent is jid-1
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 96; ind += blockDim.x*blockDim.y){
            ...
            s_deeTemp[ind] = (djid == 5) ? s_dXhom[eeIndStart + rc] : s_Xhom[eeIndStart + rc];
        }
        __syncthreads();
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
            for (int i = 0; i < 6; i++){printf("X_chain0[%d]\n",i); printMat<T,4,4>(&s_eeTemp[16*i],4);}
            for (int i = 0; i < 6; i++){printf("dX_chain0[%d]\n",i); printMat<T,4,4>(&s_deeTemp[16*i],4);}
        }
        __syncthreads();
        // Serial chain manipulator so optimize as parent is jid-1
        // Update with parent transform until you reach the base [level 1/5]
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 96; ind += blockDim.x*blockDim.y){
            ...
            s_eeTemp[ind + 96] = dot_prod<T,4,4,1>(&s_Xhom[16*4 + row], &s_eeTemp[0 + colInd]);
            const T *s_Xhom_dXhom = ((djid == 4) ? s_dXhom : s_Xhom);
            s_deeTemp[ind + 96] = dot_prod<T,4,4,1>(&s_Xhom_dXhom[16*4 + row], &s_deeTemp[0 + colInd]);
        }
        __syncthreads();
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
            for (int i = 0; i < 6; i++){printf("X_chain0[%d]\n",i); printMat<T,4,4>(&s_eeTemp[16*i + 96],4);}
            for (int i = 0; i < 6; i++){printf("dX_chain0[%d]\n",i); printMat<T,4,4>(&s_deeTemp[16*i + 96],4);}
        }
        __syncthreads();
        // Serial chain manipulator so optimize as parent is jid-1
        // Update with parent transform until you reach the base [level 2/5]
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 96; ind += blockDim.x*blockDim.y){
            int djid = ind / 16; int rc = ind % 16; int row = rc % 4; int colInd = ind - row;
            s_eeTemp[ind + 0] = dot_prod<T,4,4,1>(&s_Xhom[16*3 + row], &s_eeTemp[96 + colInd]);
            const T *s_Xhom_dXhom = ((djid == 3) ? s_dXhom : s_Xhom);
            s_deeTemp[ind + 0] = dot_prod<T,4,4,1>(&s_Xhom_dXhom[16*3 + row], &s_deeTemp[96 + colInd]);
        }
        __syncthreads();
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 96; ind += blockDim.x*blockDim.y){
            int djid = ind / 16; int rc = ind % 16; int row = rc % 4; int colInd = ind - row;
            s_eeTemp[ind + 96] = dot_prod<T,4,4,1>(&s_Xhom[16*2 + row], &s_eeTemp[0 + colInd]);
            const T *s_Xhom_dXhom = ((djid == 2) ? s_dXhom : s_Xhom);
            s_deeTemp[ind + 96] = dot_prod<T,4,4,1>(&s_Xhom_dXhom[16*2 + row], &s_deeTemp[0 + colInd]);
        }
        __syncthreads();
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 96; ind += blockDim.x*blockDim.y){
            int djid = ind / 16; int rc = ind % 16; int row = rc % 4; int colInd = ind - row;
            s_eeTemp[ind + 0] = dot_prod<T,4,4,1>(&s_Xhom[16*1 + row], &s_eeTemp[96 + colInd]);
            const T *s_Xhom_dXhom = ((djid == 1) ? s_dXhom : s_Xhom);
            s_deeTemp[ind + 0] = dot_prod<T,4,4,1>(&s_Xhom_dXhom[16*1 + row], &s_deeTemp[96 + colInd]);
        }
        __syncthreads();
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 96; ind += blockDim.x*blockDim.y){
            int djid = ind / 16; int rc = ind % 16; int row = rc % 4; int colInd = ind - row;
            s_eeTemp[ind + 96] = dot_prod<T,4,4,1>(&s_Xhom[16*0 + row], &s_eeTemp[0 + colInd]);
            const T *s_Xhom_dXhom = ((djid == 0) ? s_dXhom : s_Xhom);
            s_deeTemp[ind + 96] = dot_prod<T,4,4,1>(&s_Xhom_dXhom[16*0 + row], &s_deeTemp[0 + colInd]);
        }
        __syncthreads();
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 36; ind += blockDim.x*blockDim.y){
            ...
            T *s_Xmat_hom = &s_eeTemp[96 + 16*deeInd]; T *s_dXmat_hom = &s_deeTemp[96 + 16*deeInd];
            // xyz is easy
    ...
    void inverse_dynamics_inner(T *s_c,  T *s_vaf, const T *s_q, const T *s_qd, const T *s_qdd, T *s_XImats, T *s_temp, const T gravity) {
        ...
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("s_v[3] = X*s_v[2] + S*qd[3]\n"); printMat<T,1,6>(&s_vaf[6*3],1);
            printf("s_a[3] = X*s_a[2] + S*qdd[3]\n"); printMat<T,1,6>(&s_vaf[36 + 6*3],1);
        }
        __syncthreads();
        // sync before a += MxS(v)*qd[S] 
        __syncthreads();
        ...
        //
        for(int dof_id = threadIdx.x + threadIdx.y*blockDim.x; dof_id < 6; dof_id += blockDim.x*blockDim.y){
            s_c[dof_id] = s_vaf[72 + 6*dof_id + 2];
        }
        __syncthreads();
    }
    ...
    void direct_minv_inner(T *s_Minv, const T *s_q, T *s_XImats, T *s_temp) {
        ...
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("q\n"); printMat<T,1,6>(s_q,1);
            for (int i = 0; i < 6; i++){printf("X[%d]\n",i); printMat<T,6,6>(&s_XImats[36*i],6);}
            for (int i = 0; i < 6; i++){printf("IA_init = I[%d]\n",i); printMat<T,6,6>(&s_temp[216 + 36*i],6);}
        }
        __syncthreads();
        //
        // Backward Pass
        ...
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("U[5]\n"); printMat<T,1,6>(&s_temp[432 + 6*5],1);
            printf("Dinv[5] = %f\n",s_temp[468 + 5]);
            printf("Minv after Dinv setting before subtree\n"); printMat<T,6,6>(s_Minv,6);
        }
        __syncthreads();
        ...
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("F[4] = X^T F[5]\n");
            printMat<T,6,6>(&s_temp[144],6);
            printf("Ia*X[5]\n");
            printMat<T,6,6>(&s_temp[510],6);
        }
        __syncthreads();
        ...
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("U[4]\n"); printMat<T,1,6>(&s_temp[432 + 6*4],1);
            printf("Dinv[4] = %f\n",s_temp[468 + 4]);
            printf("Minv after Dinv setting before subtree\n"); printMat<T,6,6>(s_Minv,6);
        }
        __syncthreads();
        ...
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("U[3]\n"); printMat<T,1,6>(&s_temp[432 + 6*3],1);
            printf("Dinv[3] = %f\n",s_temp[468 + 3]);
            printf("Minv after Dinv setting before subtree\n"); printMat<T,6,6>(s_Minv,6);
        }
        __syncthreads();
        ...
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("IA[2] = X^T*(Ia*X)\n");
            printMat<T,6,6>(&s_temp[288],6);
        }
        __syncthreads();
        ...
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("U[2]\n"); printMat<T,1,6>(&s_temp[432 + 6*2],1);
            printf("Dinv[2] = %f\n",s_temp[468 + 2]);
            printf("Minv after Dinv setting before subtree\n"); printMat<T,6,6>(s_Minv,6);
        }
        __syncthreads();
        ...
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("U[1]\n"); printMat<T,1,6>(&s_temp[432 + 6*1],1);
            printf("Dinv[1] = %f\n",s_temp[468 + 1]);
            printf("Minv after Dinv setting before subtree\n"); printMat<T,6,6>(s_Minv,6);
        }
        __syncthreads();
        ...
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("U[0]\n"); printMat<T,1,6>(&s_temp[432 + 6*0],1);
            printf("Dinv[0] = %f\n",s_temp[468 + 0]);
            printf("Minv after Dinv setting before subtree\n"); printMat<T,6,6>(s_Minv,6);
        }
        __syncthreads();
        ...
        if(threadIdx.x == 0 && threadIdx.y == 0){
            ...
            printf("-------------------\n");
            printf("U\n"); printMat<T,6,6>(&s_temp[432],6);
            printf("Dinv\n"); printMat<T,1,6>(&s_temp[468],1);
            printf("F[%d]\n",0); printMat<T,6,6>(&s_temp[0],6);
            printf("F[%d]\n",1); printMat<T,6,6>(&s_temp[36],6);
            printf("F[%d]\n",2); printMat<T,6,6>(&s_temp[72],6);
            printf("F[%d]\n",3); printMat<T,6,6>(&s_temp[108],6);
            printf("F[%d]\n",4); printMat<T,6,6>(&s_temp[144],6);
            printf("F[%d]\n",5); printMat<T,6,6>(&s_temp[180],6);
            printf("Minv\n"); printMat<T,6,6>(s_Minv,6);
            printf("-------------------\n");
        }
        __syncthreads();
        ...
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("F[i,:,i:] = Xmat*F[parent,:,i:] for i[3]\n");
            printMat<T,6,6>(&s_temp[0 + 108],6);
        }
        __syncthreads();
        //   Finish this step with Minv[i,i:] -= Dinv*U^T*F[i,:,i:]
        //     and then update F[i,:,i:] += S*Minv[i,i:]
    ...
    void forward_dynamics_inner(T *s_qdd, const T *s_q, const T *s_qd, const T *s_u, T *s_XImats, T *s_temp, const T gravity) {
        ...
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("Minv\n"); printMat<T,6,6>(s_temp,6);
            printf("u\n"); printMat<T,1,6>(s_u,1);printf("c\n"); printMat<T,1,6>(&s_temp[36],1);
        }
        __syncthreads();
    ...
    void inverse_dynamics_gradient_inner(T *s_dc_du, const T *s_q, const T *s_qd, const T *s_vaf, T *s_XImats, T *s_temp, const T gravity) {
        ...
        if(threadIdx.x == 0 && threadIdx.y == 0){
            ...
            printf("q\n"); printMat<T,1,6>(s_q,1);
            printf("qd\n"); printMat<T,1,6>(s_qd,1);
            printf("vaf-v\n"); printMat<T,6,6>(s_vaf,6);
            printf("vaf-a\n"); printMat<T,6,6>(&s_vaf[6*6],6);
            printf("vaf-f\n"); printMat<T,6,6>(&s_vaf[12*6],6);
            for (int i = 0; i < 6; i++){printf("X[%d]\n",i); printMat<T,6,6>(&s_XImats[36*i],6);}
            for (int i = 0; i < 6; i++){printf("I[%d]\n",i); printMat<T,6,6>(&s_XImats[36*(i+6)],6);}
            printf("-------------------------\n");
        }
        ...
        if(threadIdx.x == 0 && threadIdx.y == 0){
            ...
            printf("-------------------------\n");
            printf("Iv\n"); printMat<T,6,6>(&s_temp[1296],6);
            printf("Xv\n"); printMat<T,6,6>(&s_temp[936],6);
        ...
        if(threadIdx.x == 0 && threadIdx.y == 0){
            ...
            printf("-------------------------\n");
            printf("Mx(Xv)\n"); printMat<T,6,6>(&s_temp[1152],6);
            printf("Mx(Xa)\n"); printMat<T,6,6>(&s_temp[1188],6);
            printf("Mx(v)\n"); printMat<T,6,6>(&s_temp[1224],6);
            printf("Mx(f)\n"); printMat<T,6,6>(&s_temp[1260],6);
            printf("-------------------------\n");
        }
        __syncthreads();
        //
        ...
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("dv/du in for bfs wave[%d]\n",3);
            printf("dv[%d]/dq\n",3);
            printMat<T,6,4>(&s_temp[36],6);
            printf("dv[%d]/dqd\n",3);
            printMat<T,6,4>(&s_temp[162],6);
        }
        __syncthreads();
        ...
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("dv/du in for bfs wave[%d]\n",4);
            printf("dv[%d]/dq\n",4);
            printMat<T,6,5>(&s_temp[60],6);
            printf("dv[%d]/dqd\n",4);
            printMat<T,6,5>(&s_temp[186],6);
        }
        __syncthreads();
        ...
        if(threadIdx.x == 0 && threadIdx.y == 0){
            ...
            printf("da[%d]/dqd\n",0);
            printMat<T,6,1>(&s_temp[378],6);
            printf("da[%d]/dq\n",1);
            printMat<T,6,2>(&s_temp[258],6);
            printf("da[%d]/dqd\n",1);
            printMat<T,6,2>(&s_temp[384],6);
            printf("da[%d]/dq\n",2);
            printMat<T,6,3>(&s_temp[270],6);
            printf("da[%d]/dqd\n",2);
            printMat<T,6,3>(&s_temp[396],6);
            printf("da[%d]/dq\n",3);
            printMat<T,6,4>(&s_temp[288],6);
            printf("da[%d]/dqd\n",3);
            printMat<T,6,4>(&s_temp[414],6);
            printf("da[%d]/dq\n",4);
            printMat<T,6,5>(&s_temp[312],6);
            printf("da[%d]/dqd\n",4);
            printMat<T,6,5>(&s_temp[438],6);
            printf("da[%d]/dq\n",5);
            printMat<T,6,6>(&s_temp[342],6);
            printf("da[%d]/dqd\n",5);
            printMat<T,6,6>(&s_temp[468],6);
        }
        __syncthreads();
        ...
        if(threadIdx.x == 0 && threadIdx.y == 0){
            ...
            printf("-------------------------\n");
            printf("df[%d]/dq\n",0);
            printMat<T,6,6>(&s_temp[504],6);
            printf("df[%d]/dqd\n",0);
            printMat<T,6,6>(&s_temp[720],6);
            printf("Fx(v)*I[%d]\n",0);
            printMat<T,6,6>(&s_temp[936 + 36*0],6);
            printf("df[%d]/dq\n",1);
            printMat<T,6,6>(&s_temp[540],6);
            printf("df[%d]/dqd\n",1);
            printMat<T,6,6>(&s_temp[756],6);
            printf("Fx(v)*I[%d]\n",1);
            printMat<T,6,6>(&s_temp[936 + 36*1],6);
            printf("df[%d]/dq\n",2);
            printMat<T,6,6>(&s_temp[576],6);
            printf("df[%d]/dqd\n",2);
            printMat<T,6,6>(&s_temp[792],6);
            printf("Fx(v)*I[%d]\n",2);
            printMat<T,6,6>(&s_temp[936 + 36*2],6);
            printf("df[%d]/dq\n",3);
            printMat<T,6,6>(&s_temp[612],6);
            printf("df[%d]/dqd\n",3);
            printMat<T,6,6>(&s_temp[828],6);
            printf("Fx(v)*I[%d]\n",3);
            printMat<T,6,6>(&s_temp[936 + 36*3],6);
            printf("df[%d]/dq\n",4);
            printMat<T,6,6>(&s_temp[648],6);
            printf("df[%d]/dqd\n",4);
            printMat<T,6,6>(&s_temp[864],6);
            printf("Fx(v)*I[%d]\n",4);
            printMat<T,6,6>(&s_temp[936 + 36*4],6);
            printf("df[%d]/dq\n",5);
            printMat<T,6,6>(&s_temp[684],6);
            printf("df[%d]/dqd\n",5);
            printMat<T,6,6>(&s_temp[900],6);
            printf("Fx(v)*I[%d]\n",5);
            printMat<T,6,6>(&s_temp[936 + 36*5],6);
        }
        __syncthreads();
        ...
        if(threadIdx.x == 0 && threadIdx.y == 0){
            ...
            printf("-------------------------\n");
            printf("-X^T*mx(f)[%d]\n",0);
            printMat<T,1,6>(&s_temp[1152 + 6*0],1);
            printf("-X^T*mx(f)[%d]\n",1);
            printMat<T,1,6>(&s_temp[1152 + 6*1],1);
            printf("-X^T*mx(f)[%d]\n",2);
            printMat<T,1,6>(&s_temp[1152 + 6*2],1);
            printf("-X^T*mx(f)[%d]\n",3);
            printMat<T,1,6>(&s_temp[1152 + 6*3],1);
            printf("-X^T*mx(f)[%d]\n",4);
            printMat<T,1,6>(&s_temp[1152 + 6*4],1);
            printf("-X^T*mx(f)[%d]\n",5);
            printMat<T,1,6>(&s_temp[1152 + 6*5],1);
        }
        __syncthreads();
        //
        // BACKWARD Pass
        ...
        if(threadIdx.x == 0 && threadIdx.y == 0){
            ...
            printf("df[%d]/dq\n",1);
            printMat<T,6,2>(&s_temp[540],6);
            printf("df[%d]/dq\n",2);
            printMat<T,6,3>(&s_temp[576],6);
            printf("df[%d]/dq\n",3);
            printMat<T,6,4>(&s_temp[612],6);
            printf("df[%d]/dq\n",4);
            printMat<T,6,5>(&s_temp[648],6);
            printf("df[%d]/dq\n",5);
            printMat<T,6,6>(&s_temp[684],6);
            printf("df[%d]/dqd\n",0);
            printMat<T,6,1>(&s_temp[720],6);
            printf("df[%d]/dqd\n",1);
            printMat<T,6,2>(&s_temp[756],6);
            printf("df[%d]/dqd\n",2);
            printMat<T,6,3>(&s_temp[792],6);
            printf("df[%d]/dqd\n",3);
            printMat<T,6,4>(&s_temp[828],6);
            printf("df[%d]/dqd\n",4);
            printMat<T,6,5>(&s_temp[864],6);
            printf("df[%d]/dqd\n",5);
            printMat<T,6,6>(&s_temp[900],6);
        }
        ...
        if(threadIdx.x == 0 && threadIdx.y == 0){
            ...
            printf("-------------------------\n");
            printf("dc/dq\n");
            printMat<T,6,6>(&s_dc_du[0],6);
            printf("dc/dqd\n");
            printMat<T,6,6>(&s_dc_du[36],6);
        }
        __syncthreads();
    }
    ...
    void inverse_dynamics_gradient_kernel_single_timing(T *d_dc_du, const T *d_q_qd, const int stride_q_qd, const T *d_qdd, const robotModel<T> *d_robotModel, const T gravity, const int NUM_TIMESTEPS) {
        __shared__ T s_q_qd[12]; T *s_q = s_q_qd; T *s_qd = &s_q_qd[6];
        __shared__ T s_qdd[6]; 
     ...
     *
     * @param s_qdd is the vector of joint accelerations
     * @param s_va is a pointer to shared memory of size 2*6*NUM_JOINTS = 72
     * @param s_q is the vector of joint positions
     * @param s_qd is the vector of joint velocities
    ...
    void aba_inner(T *s_qdd, T *s_va, const T *s_q, const T *s_qd, const T *s_tau, T *s_XImats, T *s_temp, const T gravity) {
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 6; ind += blockDim.x*blockDim.y){
            int jid = ind;
            int jid6 = 6 * jid;
            mx2_scaled<T>(&s_temp[72 * 6+jid6], &s_va[jid6], s_qd[jid]);
        }
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("c\n"); printMat<T,6,6>(&s_temp[72 * 6], 6);
        }
        __syncthreads();
        // Initialize IA = I
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 216; ind += blockDim.x*blockDim.y){
            s_temp[ind] = s_XImats[216 + ind];
        }
        // Initialize vcross[k]
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 6; ind += blockDim.x*blockDim.y){
            int jid = ind;
            int jid6 = 6 * jid;
            vcross<T>(&s_temp[36*(6+jid)], &s_va[jid6]);
        }
        __syncthreads();
        // temp[k] = -vcross.T*I[k]
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 216; ind += blockDim.x*blockDim.y){
            ...
            int jid6 = 6 * jid;
            s_temp[98 * 6 + jid6*6 + row+col*6] = -1 * dot_prod<T,6,1,1>(&s_temp[36*(6+jid)+row*6], &s_XImats[36 * (6+jid) + col*6]);
        }
        __syncthreads();
        // pA[k] = temp[k]*v[k][0]
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 36; ind += blockDim.x*blockDim.y){
            int row = ind % 6; int comp = ind / 6; int jid = comp % 6;
            int jid6 = 6 * jid;
            s_temp[78 * 6 + jid6 + row] = dot_prod<T,6,6,1>(&s_temp[98 * 6 + 6*jid6+row], &s_va[jid6]);
        }
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
            for (int i = 0; i < 6; i++){printf("IA[%d]\n",i); printMat<T,6,6>(&s_temp[36*(i)],6);}
            printf("pA\n"); printMat<T,6,6>(&s_temp[78 * 6], 6);
        }
        __syncthreads();
        //
        // Backward Pass
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 6; ind += blockDim.x*blockDim.y){
            ...
            int jid = 5;
            int jid6 = 6 * 5;
            s_temp[84*6+jid6+row] = s_temp[36*jid+row+6*(2)];
        }
        __syncthreads();
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 1; ind += blockDim.x*blockDim.y){
            int jid = 5;
            int jid6 = 6 * 5;
            s_temp[96 * 6 + jid] = s_temp[84 * 6 + jid6 + 2];
            T tempval = s_temp[78 * 6 + jid6 + 2];
            s_temp[97 * 6 + jid] = s_tau[jid] - tempval;
        }
        __syncthreads();
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 36; ind += blockDim.x*blockDim.y){
            ...
            int jid = 5;
            int jid6 = 6 * 5;
            s_temp[36 * 6+6*jid6+row+6*col] = s_temp[84*6+jid6+row]*s_temp[84*6+jid6+col]/s_temp[96 *6+jid];
            s_temp[36 * 6+6*jid6+row+6*col] = s_temp[6*jid6+row+6*col] - s_temp[36 * 6+6*jid6+row+6*col];
        }
        __syncthreads();
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 6; ind += blockDim.x*blockDim.y){
            ...
            int jid = 5;
            int jid6 = 6 * 5;
            T Uval = s_temp[84 * 6+jid6+row]*s_temp[97*6+jid]/s_temp[96*6+jid];
            s_temp[90 * 6 + jid6 + row] = s_temp[78 * 6 + jid6+row] + dot_prod<T,6,6,1>(&s_temp[36*(6+jid)+row], &s_temp[72*6+jid6]) + Uval;
        }
        // temp[k] = X[k].T*Ia[k]
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 36; ind += blockDim.x*blockDim.y){
            int row = ind % 6; int col = (ind / 6) %6;
            int jid = 5;
            int jid6 = 6 * jid;
            s_temp[98 * 6 + 6 * jid6 + row + 6*col] = dot_prod<T,6,1,1>(&s_XImats[6*jid6+6*row], &s_temp[36 * 6+jid6*6+6*col]);
        }
        __syncthreads();
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 36; ind += blockDim.x*blockDim.y){
            ...
            prodtemp =  dot_prod<T,6,6,1>(&s_temp[98 * 6 + 6 * jid6 + row], &s_XImats[6*jid6+6*col]);
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 6; ind += blockDim.x*blockDim.y){
            ...
            int jid = 5;
            int jid6 = 6 * 5;
            s_temp[134 * 6 + jid6 + row] = dot_prod<T,6,1,1>(&s_XImats[36*jid+6*row],&s_temp[90*6+jid6]);
            atomicAdd(&s_temp[78 * 6 + 6 * 4 + row], s_temp[134 * 6 + jid6 + row]);
        }
        __syncthreads();
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 6; ind += blockDim.x*blockDim.y){
            ...
            int jid6 = 6 * 4;
            s_temp[134 * 6 + jid6 + row] = dot_prod<T,6,1,1>(&s_XImats[36*jid+6*row],&s_temp[90*6+jid6]);
            atomicAdd(&s_temp[78 * 6 + 6 * 3 + row], s_temp[134 * 6 + jid6 + row]);
        }
        __syncthreads();
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 6; ind += blockDim.x*blockDim.y){
            ...
            int jid6 = 6 * 3;
            s_temp[134 * 6 + jid6 + row] = dot_prod<T,6,1,1>(&s_XImats[36*jid+6*row],&s_temp[90*6+jid6]);
            atomicAdd(&s_temp[78 * 6 + 6 * 2 + row], s_temp[134 * 6 + jid6 + row]);
        }
        __syncthreads();
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 6; ind += blockDim.x*blockDim.y){
            ...
            int jid6 = 6 * 2;
            s_temp[134 * 6 + jid6 + row] = dot_prod<T,6,1,1>(&s_XImats[36*jid+6*row],&s_temp[90*6+jid6]);
            atomicAdd(&s_temp[78 * 6 + 6 * 1 + row], s_temp[134 * 6 + jid6 + row]);
        }
        __syncthreads();
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 6; ind += blockDim.x*blockDim.y){
            ...
            int jid6 = 6 * 1;
            s_temp[134 * 6 + jid6 + row] = dot_prod<T,6,1,1>(&s_XImats[36*jid+6*row],&s_temp[90*6+jid6]);
            atomicAdd(&s_temp[78 * 6 + 6 * 0 + row], s_temp[134 * 6 + jid6 + row]);
        }
        __syncthreads();
        ...
        __syncthreads();
        if(threadIdx.x == 0 && threadIdx.y == 0){
            printf("U \n"); printMat<T,6,6>(&s_temp[84 * 6], 6);
            printf("d \n"); printMat<T,1,6>(&s_temp[96 * 6], 1);
            printf("u \n"); printMat<T,1,6>(&s_temp[97 * 6], 1);
            for (int i = 0; i < 6; i++){printf("Ia[%d]\n",i); printMat<T,6,6>(&s_temp[36*(6+i)],6);}
            for (int i = 0; i < 6; i++){printf("IA[%d]\n",i); printMat<T,6,6>(&s_temp[36*(i)],6);}
            printf("pA\n"); printMat<T,6,6>(&s_temp[78 * 6], 6);
        }
        __syncthreads();
        ...
        for(int row = threadIdx.x + threadIdx.y*blockDim.x; row < 6; row += blockDim.x*blockDim.y){
            ...
            s_va[6*6+jid6+row] = dot_prod<T,6,6,1>(&s_XImats[36 * jid + row], &gravity_vec[0]) + s_temp[72*6+jid6+row];
        }
        __syncthreads();
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 1; ind += blockDim.x*blockDim.y){
            int jid = 0;
            int jid6 = 6 * 0;
            T tempval = s_temp[97 * 6+jid] - dot_prod<T,6,1,1>(&s_temp[84*6+jid6], &s_va[6*6+jid6]);
            s_qdd[jid] = tempval / s_temp[96*6+jid];
        }
        __syncthreads();
        // a[k] += qdd[k]*S[k]
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 6; ind += blockDim.x*blockDim.y){
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 6; ind += blockDim.x*blockDim.y){
            ...
            int jid = 1;
            int jid6 = 6 * 1;
            s_va[6*6+jid6+row] = dot_prod<T,6,6,1>(&s_XImats[36 * jid + row], &s_va[6*6+(6 * 0)]) + s_temp[72*6+jid6+row];
        }
        __syncthreads();
        // qdd[k] = (u[k] - U[k].T*a[k])/d[k]
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 1; ind += blockDim.x*blockDim.y){
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 6; ind += blockDim.x*blockDim.y){
            ...
            int jid = 2;
            int jid6 = 6 * 2;
            s_va[6*6+jid6+row] = dot_prod<T,6,6,1>(&s_XImats[36 * jid + row], &s_va[6*6+(6 * 1)]) + s_temp[72*6+jid6+row];
        }
        __syncthreads();
        // qdd[k] = (u[k] - U[k].T*a[k])/d[k]
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 1; ind += blockDim.x*blockDim.y){
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 6; ind += blockDim.x*blockDim.y){
            ...
            int jid = 3;
            int jid6 = 6 * 3;
            s_va[6*6+jid6+row] = dot_prod<T,6,6,1>(&s_XImats[36 * jid + row], &s_va[6*6+(6 * 2)]) + s_temp[72*6+jid6+row];
        }
        __syncthreads();
        // qdd[k] = (u[k] - U[k].T*a[k])/d[k]
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 1; ind += blockDim.x*blockDim.y){
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 6; ind += blockDim.x*blockDim.y){
            ...
            int jid = 4;
            int jid6 = 6 * 4;
            s_va[6*6+jid6+row] = dot_prod<T,6,6,1>(&s_XImats[36 * jid + row], &s_va[6*6+(6 * 3)]) + s_temp[72*6+jid6+row];
        }
        __syncthreads();
        // qdd[k] = (u[k] - U[k].T*a[k])/d[k]
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 1; ind += blockDim.x*blockDim.y){
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 6; ind += blockDim.x*blockDim.y){
            ...
            int jid = 5;
            int jid6 = 6 * 5;
            s_va[6*6+jid6+row] = dot_prod<T,6,6,1>(&s_XImats[36 * jid + row], &s_va[6*6+(6 * 4)]) + s_temp[72*6+jid6+row];
        }
        __syncthreads();
        // qdd[k] = (u[k] - U[k].T*a[k])/d[k]
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 1; ind += blockDim.x*blockDim.y){
    ...
    void aba_device(const T *s_q, const T *s_qd, const T *s_tau, const robotModel<T> *d_robotModel, const T gravity) {
        extern __shared__ T s_XITemp[]; T *s_XImats = s_XITemp; T *s_temp = &s_XITemp[432];
        extern __shared__ T s_va[2*6*6];
        extern __shared__ T s_qdd[6];
        load_update_XImats_helpers<T>(s_XImats, s_q, d_robotModel, s_temp);
    ...
    void aba_kernel_single_timing(T *d_qdd, const T *d_q_qd_tau, const int stride_q_qd, const robotModel<T> *d_robotModel, const T gravity, const int NUM_TIMESTEPS) {
        __shared__ T s_qdd[6];
        __shared__ T s_q_qd_tau[3*6]; T *s_q = s_q_qd_tau; T *s_qd = &s_q_qd_tau[6]; T *s_tau = &s_q_qd_tau[2 * 6];
        __shared__ T s_va[72];
        extern __shared__ T s_XITemp[]; T *s_XImats = s_XITemp; T *s_temp = &s_XITemp[432];
        // load to shared mem
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 18; ind += blockDim.x*blockDim.y){
     ...
     * @param s_XImats is the (shared) memory holding the updated XI matricies for the given s_q
     * @param s_temp is a pointer to helper shared memory of size 6*NUM_JOINTS = 840
     * @param gravity is the gravity constant
     */
    template <typename T>
    __device__
    void crba_inner(T *s_M, const T *s_q, const T *s_qd, T *s_XImats, T *s_temp, const T gravity) {
        ...
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 36; ind += blockDim.x*blockDim.y){
            ...
            s_XImats[36*(parent_ind +-1+7) + col + (6*row)] += beta[6*jid6 + col + (6*row)];
        }
        __syncthreads();
        ...
        //
        for(int jid = threadIdx.x + threadIdx.y*blockDim.x; jid < 6; jid += blockDim.x*blockDim.y){
            s_M[jid+jid*6] = s_XImats[216 + 36*jid + 6*2 + 2];
        }
        __syncthreads();
        //
        ...
        for(int i = threadIdx.x + threadIdx.y*blockDim.x; i < 36; i += blockDim.x*blockDim.y){
            ...
            s_fh[i] = s_XImats[216 + 36*jid + 6*2 + ind];
        }
        for(int jid = threadIdx.x + threadIdx.y*blockDim.x; jid < 6; jid += blockDim.x*blockDim.y){
            int jid_parents[] = {-1, -1, -1, -1, -1};
            int num_parents = 0;
            switch (jid) {
                case 0:
                    num_parents += 0;
            ...
            for (int i = 0; i < num_parents; i++) {
                ...
                s_M[jid*6 + parent_ind] = s_fh[jid*6 + 2];
                s_M[parent_ind*6 + jid] = s_M[jid*6 + parent_ind];
            }
        }
        __syncthreads();
    }
    ...
    void crba_kernel_single_timing(T *d_M, const T *d_q_qd, const int stride_q_qd, const robotModel<T> *d_robotModel, const T gravity, const int NUM_TIMESTEPS) {
        __shared__ T s_M[36];
        __shared__ T s_q_qd[3*6]; T *s_q = s_q_qd; T *s_qd = &s_q_qd[6];
        extern __shared__ T s_XITemp[]; T *s_XImats = s_XITemp; T *s_temp = &s_XITemp[432];
        // load to shared mem
        for(int ind = threadIdx.x + threadIdx.y*blockDim.x; ind < 18; ind += blockDim.x*blockDim.y){
            s_q_qd[ind] = d_q_qd[ind];
