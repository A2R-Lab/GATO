...
#pragma unroll
                                  ...
                                  T                          ctrl_lim_cost)
        {
                ...
                // EE pose via the caller-scratch _inner path (migrated from the removed
                // end_effector_positions_device; the cost only reads position components [0:3]).
                // Layout mirrors grid's device wrapper: s_XmatsHom[128] + s_temp[32]; null helpers.
                // TODO(GRiD B-track): replace with a grid-emitted caller-scratch EE wrapper (magic 128/32).
                {
                        int* s_ee_topo = nullptr;
                        T* s_XmatsHom = s_scratch;
                        T* s_ee_temp = s_XmatsHom + 128;
...
#pragma unroll
        ...
        {
                // s_cost_vec (<= NX + 3) + s_eePos_cost (6) + s_scratch (EE-pose arena, >= 128+32).
                                                       ...
                                                       T        ctrl_lim_cost)
        {
                ...
                // EE pose + gradient via the caller-scratch _inner path (migrated from the removed
                // end_effector_positions_* API; cost reads position components [0:3]). s_scratch holds
                // s_XmatsHom[128] + s_temp (32 for pose, 168 for gradient; the larger bounds it). dXhom
                // computed internally (nullptr). TODO(GRiD B-track): grid-emitted caller-scratch EE wrapper.
                {
                        int* s_ee_topo = nullptr;
                        T* s_XmatsHom = s_scratch;
                        T* s_ee_temp = s_XmatsHom + 128;
                        ...
                        if (i < NX) {
                                for (int j = 0; j < NX; j++) {
                                        if (j < NQ && i < NQ) {
                                                ...
                                                s_Qk[i * NX + j] += q_lim_cost * barrier_grad_i * barrier_grad_j;

                                                // s_Qk[i * NX + j] = s_qk[i] * s_qk[j] * (blockIdx.x == KNOT_POINTS - 1 ? N_cost : q_cost);
                                                // joint barrier
                                                ...
                                                if (i == j) {
                                                        T vel_barrier_grad_i = jointBarrierGradient(s_xu[i], VEL_LIMITS<T>()[i - NQ][0], VEL_LIMITS<T>()[i - NQ][1]);
                                                        s_Qk[i * NX + j] += vel_lim_cost * vel_barrier_grad_i * vel_barrier_grad_i;
                                                }
                                        }
                                }
                        } else {
                                ...
                                for (int j = 0; j < NU; j++) { 
                                        ...
                                        if (offset == j) {
                                                ...
                                                s_Rk[offset * NU + j] += ctrl_lim_cost * ctrl_barrier_grad_i * ctrl_barrier_grad_i;
                                        }
                                }
                        }
                }
        ...
        {
                // s_eePos (6) + s_eePos_grad (6*NQ) + s_scratch (EE-gradient arena, >= 128+168=296).
