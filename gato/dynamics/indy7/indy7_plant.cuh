...
namespace gato{
	...
	namespace plant{
		...
		template <typename T>
		__device__
		void forwardDynamics(T *s_qdd, T *s_q, T *s_qd, T *s_u, T *s_XITemp, void *d_dynMem_const, T *d_f_ext){
		...
		void forwardDynamicsAndGradient(T *s_dqdd, T *s_qdd, const T *s_q, const T *s_qd, const T *s_u, T *s_temp_in, void *d_dynMem_const){
			...
			T *s_c = s_temp;
			grid::inverse_dynamics_inner<T>(s_c, s_vaf, s_q, s_qd, s_XImats, &s_temp[6], GRAVITY<T>()); __syncthreads();
