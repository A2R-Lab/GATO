...
namespace gato{
    ...
    const unsigned GRiD_SUGGESTED_THREADS = grid::SUGGESTED_THREADS; //TODO: figure out what this "suggested" for
...
// OLD
        ...
        // // Initialize constant memory needed for GRiD
		// template <typename T>
		// void *initializeDynamicsConstMem(){
		// 	grid::robotModel<T> *d_robotModel = grid::init_robotModel<T>();
		// 	return (void *)d_robotModel;
        ...
        // template <typename T, bool INCLUDE_DU = true>
		// __device__
		// void forwardDynamicsAndGradient(T *s_df_du, 
        //                                 T *s_qdd, 
        //                                 const T *s_q, 
        //                                 const T *s_qd,
		// 								const T *s_u, 
		...
		// //calculates tracking cost for a single knot point
        // template <typename T>
		// __device__
		...
		// 	// QD and R penalty
		// 	const uint32_t threadsNeeded = state_size/2 + control_size * (blockIdx.x < knot_points - 1); // state_size/2 for velocities, control_size for controls (if not last knot point)
		...
		// 	for(int i = threadIdx.x; i < threadsNeeded; i += blockDim.x){
		// 		if(i < state_size/2){ //velocity penalty
		// 			err = s_xu[i + state_size/2];
		// 			val = QD_cost * err * err;
		// 		}
		// 		else{ // controls penalty
		// 			err = s_xu[i+state_size/2];
		// 			val = R_cost * err * err;
		// 		}
		// 		s_cost_vec[i] = static_cast<T>(0.5) * val;
		// 	}
		// 	__syncthreads();
		// 	grid::end_effector_positions_device<T>(s_eePos_cost, s_xu, s_extra_temp, d_robotModel); // forward kinematics
		// 	__syncthreads();
	
		// 	// tracking error
		// 	for(int i = threadIdx.x; i < 3; i+=blockDim.x){ // TODO: add roll pitch yaw
		// 		err = s_eePos_cost[i] - s_eePos_traj[i];
		// 		s_cost_vec[threadsNeeded + i] = static_cast<T>(0.5) * err * err;
		// 	}
		// 	__syncthreads();
		...
		// 	return s_cost_vec[0]; // TODO: use warp level reduce
		// }	
